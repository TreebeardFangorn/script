...
def main():
    ...
    if st.session_state.show_input:
        st.subheader("Enter birth details for the next person")
        year = st.number_input("Year", key=f"year_{st.session_state.person_counter}", min_value=1900, max_value=2100, value=2000)
        month = st.number_input("Month", key=f"month_{st.session_state.person_counter}", min_value=1, max_value=12, value=1)
        day = st.number_input("Day", key=f"day_{st.session_state.person_counter}", min_value=1, max_value=31, value=1)
        include_time = st.checkbox("Include time (optional)", key=f"include_time_{st.session_state.person_counter}")
        if include_time:
            hour = st.number_input("Hour", key=f"hour_{st.session_state.person_counter}", min_value=0, max_value=23, value=12)
            minute = st.number_input("Minute", key=f"minute_{st.session_state.person_counter}", min_value=0, max_value=59, value=0)
        else:
            hour = 12
            minute = 0
    ...
    # Display results for all added people
    for i, person in enumerate(st.session_state.people):
        ...
        # Scores with previous people
        for j in range(i):
            other_person = st.session_state.people[j]
            scores, url = get_scores_from_user_ids(person['user_id'], other_person['user_id'])
            pos, neg, total = scores
            result = {'Positive': pos, 'Negative': neg, 'Total': total}
            compat_df = pd.DataFrame([result])
            st.write(f"With Person {other_person['id']}:")
            st.dataframe(compat_df)
            st.markdown(f"[View Compatibility Report]({url})")

    # Button to add another person
    if len(st.session_state.people) > 0 and not st.session_state.show_input:
